...
export default function Servicio({ img, service, info }: ServicioProps) {
  return (
    <div className="group relative flex h-auto min-h-screen w-full animate-fade flex-col items-center justify-center gap-12 overflow-hidden hover:justify-end hover:transition-all hover:ease-in-out sm:pt-10 lg:pt-0">
      <div className="absolute inset-0 z-30 bg-midnight/30 duration-1000 group-hover:bg-midnight/70 group-hover:transition-all group-hover:ease-in-out" />
      <Image
      ...
      <div className="z-30 hidden w-full text-pearl/90 group-hover:block group-hover:animate-fade group-hover:animate-duration-[1500ms] group-hover:animate-ease-in-out">
        {info.map((info, i) => (
          <div
            className="flex w-full flex-col items-center justify-center gap-2 border-t border-pearl bg-midnight/70 p-8 font-thin"
            key={i}
          >
        ...
        <button className="flex w-full flex-col items-center justify-center gap-2 border-t border-pearl bg-midnight/70 p-8 font-semibold duration-300 ease-in-out hover:bg-pearl hover:text-midnight">
          RESERVÁ TU CLASE
        </button>
      </div>
    </div>
